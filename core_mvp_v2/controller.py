...
from core_mvp_v2.gating import GatingNet
...
class Controller:
    ...
    def init_action_net(self):
        self.action_net = torch.nn.Sequential(
            torch.nn.Linear(self.obs_dim, 8),
            torch.nn.ReLU(),
            torch.nn.Linear(8, 1)
        )
        self.action_optimizer = torch.optim.Adam(self.action_net.parameters(), lr=1e-3)

    def act(self, obs):
        if self.action_net is None:
            return random.randint(0, 1), None
        s = torch.tensor(obs, dtype=torch.float32).unsqueeze(0)
        logit = self.action_net(s)
        prob = torch.sigmoid(logit)
        action = torch.bernoulli(prob)
        log_prob = torch.distributions.Bernoulli(prob).log_prob(action)
